...
class Flexyapress_API{
    ...
    public function get_all_cases($force = false, $amount = 9999, $updated_after = "1999-01-01"){
        $updated_since = $force ? '1999-01-01' : (get_transient('flexyapress_last_update') ?: '1999-01-01');
        $amount = $force ? 9999 : 999;

        $start = microtime(true);
        //echo 'Cases: '.  0 . '<br>';
        $api_cases = $this->get_cases($amount, $updated_since);
        //echo 'Rental Cases: '. (microtime(true) - $start).'<br>';
        $api_rental_cases = $this->get_rental_cases($amount, $updated_since);
        //echo 'Business Cases: '. (microtime(true) - $start).'<br>';
        $api_business_cases = $this->get_business_cases($amount, $updated_since);
	...
	public function order($input){
        ...
        switch ($qs['buyerActionType']){
            ...
            case 'DOCUMENTS_ORDER':
                $consent_id = $input['contactAccepted'] ? $input['consentIdGlobalWithContact'] : $input['consentIdGlobalWithoutContact'];
                $response = $this->order_documents($input['contactAccepted'], $qs['caseNo'], $qs['shopNo'], $consent_id, $qs['name'], $qs['lastName'], $qs['phone'], $qs['email'], $qs['message']);
                break;
            case 'MAKEABID_ORDER':
            ...
            case 'VALUATION_ORDER':
                $response = $this->order_sales_valuation($qs['consentIdGlobal'], $qs['shopNo'], $qs['name'], $qs['lastName'], $qs['phone'], $qs['email'], $qs['message'], $qs['livesOnAddress'], $qs['dawa-address-id'], $qs['addressRoadName'], $qs['addressZipCode'],$qs['addressZipCodeCity']);
                break;
        ...
        */
		...
		if($response['response']['code'] == 200 && empty($response_body->errors) && !empty($id) && empty($errors)){
            $return = $response;
            new PB_Log('order_success_'.$qs['buyerActionType'], $qs, serialize($response_body->data));
		}else{
			new PB_Log('order_fail_'.$qs['buyerActionType'], serialize(array('fields' => $input)), preg_replace('/[^A-Za-z0-9\-]/', '', $errors[0]->message));
    ...
    private function order_sales_valuation($consent_id, $shopNo = '', $firstName, $lastName = '', $phone, $email, $message = '', $lives_on_address = false, $dawa_guid = '', $roadname = '', $zipcode = '', $city = ''){
        $args = $this->get_header_args();
	...
	public function set_search_agent($input){
		...
		if($response['response']['code'] == 200){
			...
			//new PB_Log('set_search_agent_success', serialize(array('url' => $url, 'args' => $args, 'fields' => $input)), serialize($response));
		}else{
			new PB_Log('set_search_agent_fail', serialize(array('url' => $url, 'args' => $args, 'fields' => $input)), serialize($response));
			$return = array(
				'type' => 'error',
    ...
    public function update_lead_types(){
        $lead_types = $this->get_lead_types();
        ...
        if(!empty($lead_types)) {
            ...
            foreach ($lead_types as $lt) {
                ...
                $consents = $this->get_lead_consents_by_typeid($lt->id);
