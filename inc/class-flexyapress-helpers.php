...
class Flexyapress_Helpers{
    ...
    public static function sort_case_media($c, &$images, &$floorplans, &$videos){

        if(!empty($c->media) && !empty($c->media->items)){

            $image_arr = self::sort_mw_images_by_priority($c->media->items);
            ...
            foreach ($image_arr as $img){
                ...
                if($img->mediaType->name == 'Billede'){
                    $temp = [
                        'url' => $img->sizeDesktop,
                        'url_tablet' => $img->sizeTablet,
                        'url_mobile' => $img->sizeMobile,
                        'description' => $img->description,
                        'priority' => $img->priority
                    ];
                }else if($img->mediaType->name == 'Plantegning'){
                    $temp = [
                        'url' => $img->resourceUrl,
                        'url_tablet' => $img->sizeTablet,
                        'url_mobile' => $img->sizeMobile,
    ...
    public static function format_openhouse_dates($openhouses){
        $sorted_appointments     = [];
        if ( ! empty($openhouses)) {
    ...
    public static function get_broker($entry){
        $broker = false;

        if ( ! empty($entry->brokers) && is_array($entry->brokers)) {
            foreach ($entry->brokers as $b) {
    ...
    public static function format_property_class_from_type($type){
